defmodule Pythelix.Scripting.Runner do
  ...
  """
  @spec run_method(Method.t() | {Entity.t(), String.t()}, list(), map() | Dict.t(), list()) ::
          :nomethod | :ok
  ...
  def resume_task(task_id) do
    case Task.get(task_id) do
      ...
      %Task{action: {:entity_method, entity_key, method_name}} = task ->
        ...
        case Record.get_entity(entity_key) do
          nil ->
            Logger.warning(
              "Scheduled entity #{entity_key} no longer exists, skipping #{method_name}"
            )
      ...
      task ->
        ...
        case Script.execute(resumed_script, task.code, task.name) do
          %Script{parent: %Script{} = parent, pause: :immediately, error: nil} =
              script_with_parent ->
            parent =
              Script.put_stack(parent, script_with_parent.last_raw)
              |> then(&%{&1 | pause: nil})
    ...
    exception ->
      Logger.error(
        "Task resumption failed for task #{task_id}: " <>
  ...
  defp maybe_set_step(script, {module, function, args})
  ...
  defp look_for_next_step(%Script{step: nil, parent: nil} = script, traceback),
    do: %{script | error: traceback}

  defp look_for_next_step(%Script{step: nil, parent: parent}, traceback),
    do: look_for_next_step(parent, traceback)
